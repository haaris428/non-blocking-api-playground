...
public class BlockingAPITests {
    private static final Logger logger = Logger.getLogger(BlockingAPITests.class);
    private static final String baseUri="http://localhost:8081/blocking/resource";

    @Test
    public void testBlockingAPI() throws InterruptedException, ExecutionException {
        ...
        for (int i=0; i < parallelTasks; i++){
            completableFutureList.add(CompletableFuture.runAsync(() -> {
                try {
                    CompletableFuture<HttpResponse<String>> response = client.sendAsync(HttpRequest.newBuilder().uri(URI.create(baseUri)).build(),
